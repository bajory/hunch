...
(function configureEvergage() {
    try {
       Evergage.configure({
           account: "lululemoncanada",
           dataset: "northamerica",
           cdnUrl: "https://cdn.evergage.com",
           trackerUrl: "https://lululemoncanada.us-4.evergage.com",
           siteConfigVersion: "29"
       },{"allowBotTraffic":false,"corsAllowedOrigins":["*"],"defaultCurrency":"USD","defaultLocale":null,"disableQtipWindowScroll":false,"doNotStoreCookiesRequireProvidedAnonId":false,"doNotTrackPingRequestsForActions":false,"enableCorsRestrictedOrigins":false,"enableCrossDomainIframeCookies":false,"enableMessageRotation":false,"enableRememberMeUserIds":false,"enableTemplateEsc":false,"hideContentSections":false,"hideContentSectionsMillis":2500,"hidePagesForRedirect":false,"hidePagesForRedirectMillis":1000,"identityAttributes":["customerId","emailAddress","hashedEmail","idfv","profileForceMergeId","profileId","sfmcContactKey"],"lastModified":1689021757791,"preventSensitiveDataCapture":false,"rememberMeUserIdsMillis":63072000000,"secureCookie":false,"sendErrorEvents":true,"showPoweredBy":false,"siteConfigExecBeforePageReady":false,"spaRouteChangeTimeout":500,"trackAnonymousVisitors":true,"trackContextualRelatedItems":false,"trackSubdomainAsCompany":false,"trackUnknownPagesByTitle":false,"treatHashChangeAsPageLoad":false}       );
    } catch (e) {
          console.error(e);
    }
})();
...
SalesforceInteractions.init({
    ...
    //setTimeout(function() {
        let config = {
            global: {
                onActionEvent: (actionEvent) => {
                    ...
                    // checks for logged in user and attaches uemh to each sendEvent
                    const webUser = window?.digitalData?.user?.[0];
                    if (webUser?.uemh) {
                        actionEvent.user.attributes.hashedEmail = webUser.uemh;
                    }

                    // checks if user has an ongoing cart
                    const webCartId = window?.digitalData?.cart?.cartID;
                    if (webCartId) {
                        actionEvent.user.attributes.webCartId = webCartId;
                    }
        ...
        const selectedSize = () => {
            ...
            return params.has('sz') && params.has('color');
        }

        /*Check for URL change every half second. If URL has changed and includes checkout confirmation, reinitialize beacon and sitemap. */

        const checkForConfirmationChange = () => {
            ...
            setInterval(() => {
                ...
                if (url !== windowUrl) {
                    ...
                    if (windowUrl.includes('/checkout/confirmation')) {
        ...
        checkForConfirmationChange();

        // Function to retrieve hashed email from JWT token in the cookie
const checkForHashedEmailInToken = () => {
    function retrieveHashedEmailFromToken() {
        return new Promise((resolve) => {
            try {
                // Get JWT token from the cookie
                const token = getCookie('lll_oidc_token');
                if (!token) return resolve("");

                // Decode JWT token and extract 'sub' value
                ...
                crypto.subtle.digest("SHA-256", buffer)
                    .then(hash => resolve(hex(hash)))
                    .catch(() => resolve("")); // Resolve with empty string if hashing fails
            } catch {
                resolve(""); // Resolve with empty string if any error occurs
            }
        });
    }

    let checkForConsentStatus = () => {
    // console.log("Evergage Consent Check Running")
  // This function reads the OptanonConsent cookie and checks the value of group C0004 (1 = opt-in, 0 = opt-out)
  ...
  for (let cookie of cookies) {
    ...
    if (cookie.startsWith('OptanonConsent=')) {
      cookieValue = decodeURIComponent(cookie.substring('OptanonConsent='.length));
      break;
    }
  }

  //console.log("Evergage OptanonConsent raw value:", cookieValue);
  if (!cookieValue) return false;

  // Extract "groups" param from cookie
  const params = new URLSearchParams(cookieValue.replace(/&/g, '&'));
  const groups = params.get("groups");
  if (!groups) return false;

  // Parse group values like C0004:1,C0001:1,...
  const groupMap = {};
  ...
  // Return true if C0004 is 1 (opt-in), otherwise false
  if(groupMap["C0004"] === "0"){
    //   console.log("Evergage Status is turned off. Submitting request to Personalization to turn tracking off.")
    ...
    //   console.log("Evergage Cookie Status is Opted In. No action necessary.")
      const consent = {
                    provider: "Consent Provider",
                    purpose: SalesforceInteractions.mcis.ConsentPurpose.Personalization,
    ...
    function hex(buffer) {
        ...
        for (let i = 0; i < view.byteLength; i += 4) {
            ...
            digest += ("00000000" + value.toString(16)).slice(-8);
        }
        return digest;
    }

    // Call retrieveHashedEmailFromToken and send event if email is found
    retrieveHashedEmailFromToken().then(hashedEmail => {
        if (hashedEmail) {
            SalesforceInteractions.sendEvent({
                interaction: {
                    name: 'Hashed Email Capture from Cookie'
                },
                user: {
                    identities: {
...
checkForHashedEmailInToken();

        let checkForHashedEmail = () => {
           let params = (new URL(document.location)).searchParams;
           let hashedEmail = params.get('hashed_email');

           if (hashedEmail) {
              SalesforceInteractions.sendEvent({
                interaction: {
                    name: 'Hashed Email Capture'
                    },
                user: {
                    identities: {
                        hashedEmail: hashedEmail
        ...
        let checkForISMCID = () => {
           let params = (new URL(document.location)).searchParams;
           let ismcid = params.get('ismcid');

           if (ismcid) {
              SalesforceInteractions.sendEvent({
                interaction: {
                    name: 'SFMC ISMCID Capture'
                    },
                user: {
                    identities: {
                        sfmcContactKey: ismcid
                    }
                  }
              })
           }
        ...
        let getMyBagListeners = () => {
            let myBagListeners = [];

            let result = (new Promise(myBagMatchPromise)).then(() => {
                let bagProductsListener = SalesforceInteractions.listener('click', 'div[data-testid="productgroup"]', () => {

                    SalesforceInteractions.sendEvent({
                        interaction: {
                            name: 'Modified Bag Product'
                        }
                    });
                ...
                myBagListeners.push(bagProductsListener);

                let savedProductsListener = SalesforceInteractions.listener('click', 'div[data-testid="my-saved-items"]', () => {
                    SalesforceInteractions.sendEvent({
                        interaction: {
                            name: 'Modified Saved for Later Product'
                        }
                    });
                ...
                myBagListeners.push(savedProductsListener);

                let removeFromBagListener = SalesforceInteractions.listener('click', 'button[data-lulu-track="my-bag-product-remove-modal-true"]', () => {
                    let cartItemDetails = JSON.parse(SalesforceInteractions.cashDom('button[data-lulu-track="my-bag-product-remove-modal-true"]').attr('data-lulu-attributes')).product;
                    SalesforceInteractions.sendEvent({
                        interaction: {
                            name: SalesforceInteractions.CartInteractionName.RemoveFromCart,
                            lineItem: {
                                catalogObjectType: 'Product',
                                catalogObjectId: cartItemDetails.skuID,
                ...
                myBagListeners.push(removeFromBagListener);

                let checkoutListener = SalesforceInteractions.listener('click', 'div[class*="orderctas"] button[data-lulu-track="my-bag-checkout-enabled"]', () => {
                    SalesforceInteractions.sendEvent({
                        interaction: {
                            name: 'Clicked Lulu Checkout'
                        }
                    });
                });

                myBagListeners.push(checkoutListener);

                let paypalListener = SalesforceInteractions.listener('click', '#braintree-paypal-button', () => {
                    SalesforceInteractions.sendEvent({
                        interaction: {
                            name: 'Clicked Paypal Checkout'
                        }
                    });
                });

                myBagListeners.push(paypalListener);
        ...
        /********************************* Start Page Type Identification *********************************/
            ...
            function pdpMatchPromise(resolve, reject) {
                let counter = 0;
                let chosenOption;
                ...
                let interval = setInterval(() => {
                    ...
                    if (counter > 7) {
                        clearInterval(interval);
                        reject('page took to long to load');
                    }

                    chosenOption = SalesforceInteractions.cashDom('div[role="radio"][aria-checked="true"] picture img');

                    if (chosenOption.length > 0) {
                        if (chosenOption[0].hasAttribute('data-lulu-attributes')) {
                            clearInterval(interval);
                            resolve(true);
                        } else if (chosenOption[0].hasAttribute('srcset')) {
                            if (chosenOption[0].getAttribute('srcset').indexOf('/thumb_') > 0) {
                                clearInterval(interval);
                                resolve(true);
                            } else if (SalesforceInteractions.cashDom('#product-image-carousel-image-0 picture img')) {
                                clearInterval(interval);
                                resolve(true);
                            } else if (SalesforceInteractions.cashDom('div[class^="product-mobile-carousel_container"] picture img')) {
                                clearInterval(interval);
                                resolve(true);
                            }
                        } else if (SalesforceInteractions.cashDom('#product-image-carousel-image-0 picture img')) {
            ...
            config.pageTypes.push({
                ...
                isMatch: () => {
                    if ((window.location.pathname).includes('/p/')) {
                        let result = (new Promise(pdpMatchPromise)).then(() => {
                            SalesforceInteractions.sendEvent({
                                interaction: {
                                    name: 'Viewed PDP Web'
                                }
                ...
                interaction: {
                    ...
                    catalogObject: {
                        type: "Product",
                        id: () => {
                                return window.digitalData.product[0].productInfo.skuId
                        },
                        relatedCatalogObjects: {
                            ProductId: () => {
                            ...
                            SKUColorCodeDesc: () => {
                                return [window.digitalData.product[0].productInfo.color];
                            }/*,
                            SkuStyleOption: () => {
                                let chosenOption = SalesforceInteractions.cashDom('div[role="radio"][aria-checked="true"] picture img');
                                let chosenStyleAttributes;
                                let chosenStyle;

                                if (chosenOption.length > 0) {
                                    if (chosenOption[0].hasAttribute('data-lulu-attributes')) {
                                        chosenStyleAttributes = JSON.parse(chosenOption[0].getAttribute('data-lulu-attributes'));
                                        if (chosenStyleAttributes != null) {
                                            chosenStyle = chosenStyleAttributes.product.styleID;
                                        } else {
                                            // JSON.parse is null
                                        }
                                    } else if (chosenOption[0].hasAttribute('srcset')) {
                                        if (chosenOption[0].getAttribute('srcset').indexOf('/thumb_') > 0) {
                                            chosenStyleAttributes = chosenOption[0].getAttribute('srcset');
                                            chosenStyle = chosenStyleAttributes.substring(chosenStyleAttributes.indexOf('/thumb_')+7, chosenStyleAttributes.indexOf('?wid'));
                                        } else if (SalesforceInteractions.cashDom('#product-image-carousel-image-0 picture img').length > 0) {
                                            chosenStyleAttributes = SalesforceInteractions.cashDom('#product-image-carousel-image-0 picture img');
                                            chosenStyleAttributes = chosenStyleAttributes[0].getAttribute('srcset');

                                            chosenStyle = chosenStyleAttributes.substring(chosenStyleAttributes.indexOf('/lululemon/')+11, chosenStyleAttributes.indexOf('?wid')-2);
                                        } else {
                                            chosenStyleAttributes = SalesforceInteractions.cashDom('div[class^="product-mobile-carousel_container"] picture img');
                                            chosenStyleAttributes = chosenStyleAttributes[0].getAttribute('srcset');
                ...
                listeners: [
                    SalesforceInteractions.listener('click', 'div[aria-labelledby="pdp-size-selector-button-tile-group"]', () => {
                        SalesforceInteractions.reinit();
                    }),

                    SalesforceInteractions.listener('click', 'div[class*="color-selection_colorSelector"]', () => {
                        SalesforceInteractions.reinit();
                    }),

                    SalesforceInteractions.listener('click', 'button[class*="add-to-bag-button"]', () => {
                        if(selectedSize()){
                                     let cartEvent = SalesforceInteractions.sendEvent({
                                interaction: {
                                    name: SalesforceInteractions.CartInteractionName.AddToCart,
                                    lineItem: {
                                        catalogObjectType: "Product",
                                        catalogObjectId: window.digitalData.product[0].productInfo.skuId,
                                        quantity: 1,
                                        price: window.digitalData.product[0].productInfo.price,
                                        currency: () => {
                                            if ((window.digitalData.product[0].productInfo.skuId).includes('ca_')) {
                                                return 'CAD';
                                            } else if ((window.digitalData.product[0].productInfo.skuId).includes('us_')) {
                                                return 'USD';
                                            } else {
                                                return 'USD';
                                            }
            ...
            function cdpMatchPromise(resolve, reject) {
                let counter = 0;
                let breadCrumbs;

                let interval = setInterval(() => {
                    breadCrumbs = SalesforceInteractions.cashDom('h1.lll-text-small.OneLinkTx').text();
                    if (breadCrumbs) {
                        clearInterval(interval);
                        resolve(true);
                    }
            ...
            config.pageTypes.push({
                ...
                isMatch: () => {
                    if (window.location.pathname.includes('/c/')) {
                        let result = (new Promise(cdpMatchPromise)).then(() => {
                            SalesforceInteractions.sendEvent({
                                interaction: {
                                    name: 'Viewed CDP Web'
                                }
                ...
                interaction: {
                    ...
                    catalogObject: {
                        type: "Category",
                        id: () => {
                            let categoryTitle = digitalData["page"]["category"]["categoryName"] || ""
            ...
            config.pageTypes.push({
                name: "Story Page",
                isMatch: () => {
                    if (window.location.pathname.includes('/story/') || window.digitalData.page.category.pageType === 'story') {
                        return true;
                    }
                    return false;
                },
                interaction: {
                    name: () => {
                        ...
                        if (pageName) {
                            return 'Viewed ' + pageName + ' Story Page';
                        } else {
                            let pageUrlPath = window.location.pathname.split('/');
                            let pageUrlName = pageUrlPath[pageUrlPath.length - 1];
                            pageUrlName = pageUrlName.toLowerCase().replace(/(?:^|[\s-/])\w/g, function (match) {
                                return match.toUpperCase();
                            }).replaceAll('-', ' ');

                            return 'Viewed ' + pageUrlName + ' Story Page';
                        }
                    }
                },
            });

            function myBagMatchPromise(resolve, reject) {
                let counter = 0;
                let bagProducts;
                let savedProducts;
                ...
                let interval = setInterval(() => {
                    ...
                    bagProducts = SalesforceInteractions.cashDom('div[data-testid="productgroup"] article');
                    savedProducts = SalesforceInteractions.cashDom('div[data-testid="my-saved-items"] article');

                    if (bagProducts && savedProducts) {
                        clearInterval(interval);
                        resolve(true);
                    }
                    counter++;
            ...
            config.pageTypes.push({
                name: "My Bag",
                isMatch: () => {
                    if (window.location.pathname.includes('/mybag')) {
                        let result = (new Promise(myBagMatchPromise)).then(() => {
                            return true;
                        }, () => {
                            return false
                ...
                interaction: {
                    name: 'Viewed My Bag Page'
                },
                listeners: getMyBagListeners()
            });
            ...
            config.pageTypes.push({
                ...
                isMatch: () => {
                    if (window.location.pathname.includes('/checkout/confirmation')) {
                        SalesforceInteractions.sendEvent({
                            interaction: {
                                name: 'Viewed Checkout Confirmation'
                            }
                        });
                ...
                interaction: {
                    ...
                    order: {
                        ...
                        lineItems: () => {
                            let lineItems = [];
                            let dataLayerItems = window.digitalData.transaction.item;

                            for (let i = 0; i < dataLayerItems.length; i++) {
                                let lineItem = {};

                                lineItem.catalogObjectType = 'Product';
                                lineItem.catalogObjectId = dataLayerItems[i].productInfo.sku;
                                lineItem.quantity = dataLayerItems[i].quantity;
                                lineItem.price = dataLayerItems[i].price.netPrice / lineItem.quantity;
                        ...
                        totalValue: () => {
                            if (window.digitalData.transaction.total) {
                                return window.digitalData.transaction.total.netPrice
                            }
            ...
            config.pageTypes.push({
                ...
                listeners: [
                    SalesforceInteractions.listener('click', 'button[data-lulu-track="account-login"]', () => {
                        // Give time for Datalayer to update
                        setTimeout(function() {
                            let webUser = window.digitalData.user[0];

                            if (webUser.uemh) {
                                SalesforceInteractions.sendEvent({
                                    interaction: {
                                        name: 'Guest Logged In Web',
                                        user: {
                                            identities: {
                                                hashedEmail: webUser.uemh
                                            }
                                        }
                                    }
                                });
        ...
        function validLocalePromise(resolve, reject) {
            let counter = 0;
            let locale;

            let interval = setInterval(() => {

                locale = window?.digitalData?.page?.pageInfo?.country;
        ...
        let validLocale = (new Promise(validLocalePromise)).then(() => {
            return true;
        }, () => {
            return false
        });
